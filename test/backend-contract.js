...
const SHEETS = {
  ...
  'Challenge Ranking': [
    ...
    ['Rookie Kid','FR - Creative Tech 2','<6 months',1000,0.1,500,0.1,0,3,2],   // is_rookie + yellow (0<3<5, has activity)
    ['No Data Nils','LUXEMBOURG','',0,0,0,0,0,0,0],                              // meetings=0 AND no activity -> NOT carded
    ['Active Slacker','LUXEMBOURG','Over a year',500000,0.3,200000,0.3,0,0,4],   // meetings=0 BUT has sales -> carded
...
eq('people count (Morocco filtered)', p.people.length, 4);
const byName = Object.fromEntries(p.people.map(x => [x.name, x]));
eq('is_rookie derived', byName['Rookie Kid'].is_rookie, true);
// Discipline rule: meetings < 5 AND the person has any activity (so a fresh, all-zero
// row isn't carded just because the challenge just started).
eq('yellow_meetings: 0<3<5 with activity -> carded', byName['Rookie Kid'].yellow_meetings, true);
eq('yellow_meetings: meetings=0 AND no activity -> NOT carded', byName['No Data Nils'].yellow_meetings, false);
eq('yellow_meetings: meetings=0 BUT has sales -> carded', byName['Active Slacker'].yellow_meetings, true);
